...
- `llm.score_scale` / `tie_break_with_analytic` / `tie_break_ref_cost` — Bid elicitation. A 0-100 rating ties 59-92% of the time, so the bid cannot order agents; `score_scale` widens the range the model is asked for (the bound is in the JSON schema too, since Ollama's structured output drives from it), and `tie_break_with_analytic` orders surviving ties by the analytic cost with a term capped at half a rating step, so it can never reorder distinct ratings. Both default to the measured baseline. `[STATS]` reports bid count, distinct values and modal share
