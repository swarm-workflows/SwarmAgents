...
**Context age at decision time** is the metric that did not previously exist in any form, and the conference paper's only interaction claim (F6) cannot be produced without it. `GroupSnapshot` now carries `observed_at` / `oldest_observed_at`, the newest and oldest child `last_updated` behind the group's numbers — both, because a group of nine where one child went quiet is genuinely fresh on one and stale on the other. The age is taken at the **decision**, not when the view was built: under `delegation.policy: llm` those are seconds apart, and `decide_s` is stored alongside so the age at build is `age - decide_s`. Three things that would have made the column quietly wrong and are pinned by tests: an unknown age (no live child, or `AgentInfo.last_updated` still at its 0.0 default) is counted in `ctx_unknown_groups`, never substituted with a large one — the naive reading is a 56-year age landing in the p99 exactly where the dead groups are; a negative age is clock skew, clamped to 0 and counted in `ctx_skewed_ages`, which is a **validity** column and not a result (non-zero across a fleet means the age axis is skew, so check it first on any 17-site run); and the two new `GroupSnapshot` fields are provably invisible to `ContextExtractor`, since a changed `schema_version` discards every persisted LinUCB model. The record is written by `_delegate_child_groups`, a wrapper *around* `_select_child_groups` rather than inside it, because `LlmAgent` re-enters the bandit on five fallback routes — so there is exactly one record per delegation and it names the rule that actually decided (`llm`, `bandit`, `bandit_all`/`all` for a fan-out covering every candidate, `random`). Exported to `metrics.json` as `instrumentation` + `delegation_decisions`, summarised on the `[STATS]` line, and collected by `evaluation/collect.py` into per-run summary columns (fleet **sums**, not per-agent means, whose denominator would vary with metrics completeness) plus `decisions.csv` — one row per decision, which is what F6 plots and what the oracle (P1-1) labels offline
