...
- `job_selection.analytic_cost_half` / `llm_cost_half` — Native cost each decision plane calls middling, mapped to 50 on the canonical 0-100 wire scale (`swarm/agents/cost_scale.py`). One curve, `100*c/(c+half)`, is applied to both planes, so costs advertised to peers are comparable across them; native costs, and therefore selection and `selection_threshold_pct`, are untouched. Note the cost reaching the wire is post load-penalty (up to ~2x), which is why the LLM plane is squashed rather than clamped
