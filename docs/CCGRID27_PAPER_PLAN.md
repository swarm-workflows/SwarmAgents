...
| **F1/F2 framing** (open, deliberately) | `# TEMP HACK` in `scheduling_main`: at `level > 0` the cost matrix is over `[self]`, so every coordinator proposes itself for every job. Left unchanged — it is a design decision that would move every hierarchical number | The coordinator-tier collapse may be "n proposals per job" as much as PBFT. Report proposals/job by tier in E5 and name the behaviour in §III |
