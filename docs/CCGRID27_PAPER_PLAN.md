...
| **E1′, E3a, T1 (2026-09-18)** | Hierarchical `all_jobs.csv` uses the **delegation** time as `submitted_at` and the leaf tier's selection stamps, so `job_latency`, `makespan`, `throughput`, `sched_latency` and the headline `selection_*` all exclude the coordinator tier (`plotting/data.py:267-268`). The tier where PBFT collapses shows only in completion % and `l1_selection_*` | Use the top-level submission for the `level=None` export, add `selection_total`, and name the level each figure plots. Also decide the completion definition (`completed_at > 0` counts exit failures as complete). Review §3, §9 |
| **Every hierarchical cell (2026-09-18)** | `selection_main`'s startup barrier has no timeout: one agent that never registers idles its whole group for the run. And coordinators never detect a dead child group by staleness (only by Redis TTL, 600 s), so F5's re-adoption time is delegation-timeout learning, not liveness | Bounded barrier; staleness pruning of existing child entries. Review §4, §5 |
