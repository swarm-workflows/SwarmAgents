...
| **Every hierarchical cell (2026-09-18)** | ~~`selection_main`'s startup barrier has no timeout~~ **barrier FIXED 2026-09-20** (`selection_barrier_s`, default `failure_threshold_seconds`; `barrier_short_agents` on the collected row is a validity column). Still open: coordinators never detect a dead child group by staleness (only by Redis TTL, 600 s), so F5's re-adoption time is delegation-timeout learning, not liveness | Staleness pruning of existing child entries. Review §5 |
