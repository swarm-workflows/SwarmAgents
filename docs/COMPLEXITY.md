...
2. **Argmin-only proposal** -- an agent proposes for a job only when it computes *itself* as the minimum-cost assignee, and defers the job otherwise. This used to be attributed to a `selection_threshold_pct` tolerance around the minimum; that key never had any effect and was removed on 2026-09-20 (code review §7). The bound is therefore tighter than described, not looser. What keeps the measured figure above 1 is that each agent computes the matrix from its own view of its peers, so several can each believe they are cheapest -- ~3.7 bidders per job measured. `job_selection.designate_bidder` cuts that to ~1, but only for LLM agents — `_designate_bidders` runs in `LlmAgent.selection_main` alone, so a rule-based fleet has no knob here in either direction.
