...
- **This is failover, not fan-out — it does not give a delegation policy anything to decide.** A group is led by its lowest-ID live co-parent, so with the fleet healthy, raising K concentrates leadership rather than spreading choice. Measured on Hier-30 (5 coordinators, all alive), groups *actively led* per coordinator are `[1,1,1,1,1]` at K=1, `[0,1,1,1,2]` at K=2, and `[0,0,0,0,5]` at K=5 — at every K, at most one coordinator has more than one candidate, and at high K the rest are idle standbys. Since `scheduling_main` filters candidates through `_get_active_child_groups()`, a coordinator leading one group has one candidate: the MAB returns its only arm and `delegation.policy: llm` short-circuits without calling the model. Coordinators in that state log `[DELEGATION] ... can never choose`. Measuring learned or LLM delegation needs a coordinator that *exclusively* parents several groups: that is `--groups-per-coordinator G`, a separate flag. The two compose — G=2 with K=2 on Hier-30 gives leadership `[3, 2, 0]`, a real choice for two coordinators plus one warm standby — so a delegation cell can still exercise failover.
