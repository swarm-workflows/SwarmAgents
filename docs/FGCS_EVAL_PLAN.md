...
| F-19 | `peer_expiry_seconds` filtered only at **insertion**: an entry already in a map was removed only when its Redis key was gone, and that key's TTL is twice the threshold | A coordinator kept a dead child group for ~600 s with a frozen record reading idle, so **F5's time-to-re-adoption was delegation-timeout learning plus a Redis TTL, not liveness detection**, and E2b's liveness gate did not gate | ✅ 2026-09-20: existing entries are evicted at the staleness threshold (`_staleness_eviction_threshold`), **floored on the agent's own tier** at `failure_threshold_seconds` so eviction cannot outrun `_detect_failed_agents`, which is what reassigns a dead peer's jobs. `tests/test_child_staleness.py` (11; 7 fail pre-fix). No F5 re-adoption number from before this date is citable |
...
peers under `broadcast()` although the docs call it advisory (fixed 2026-09-20); `selection_threshold_pct` is inert (removed 2026-09-20).
