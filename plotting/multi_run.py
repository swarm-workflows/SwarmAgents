        ...
        for config_name, config_data in self.data.items():
            ...
            for run_data in config_data['runs']:
                ...
                record = {
                    ...
                    'mean_selection_time': safe_mean(df_jobs['selection_time']),
                    'median_selection_time': safe_median(df_jobs['selection_time']),
                    'p95_selection_time': safe_quantile(df_jobs['selection_time'], 0.95),
                    'p99_selection_time': safe_quantile(df_jobs['selection_time'], 0.99),

                    # Wait time metrics
                    'mean_wait_time': safe_mean(df_jobs['wait_time']),
                    'median_wait_time': safe_median(df_jobs['wait_time']),

                    # Execution metrics
                    'mean_execution_time': safe_mean(df_jobs['execution_time']),
                    'total_execution_time': safe_sum(df_jobs['execution_time']),
                    ...
                    'leader_entropy': calculate_entropy(df_jobs['leader_id']),
                }
                ...
                if topology == 'hierarchical':
                    for level in sorted(df_jobs['hierarchy_level'].unique()):
                        ...
                        record[f'level{level}_mean_selection_time'] = safe_mean(df_level['selection_time'])
                        record[f'level{level}_median_selection_time'] = safe_median(df_level['selection_time'])
                        record[f'level{level}_p95_selection_time'] = safe_quantile(df_level['selection_time'], 0.95)
                ...
                if 'latency' in run_data:
                    ...
                    record['mean_scheduling_latency'] = safe_mean(df_latency['scheduling_latency'])
                    record['median_scheduling_latency'] = safe_median(df_latency['scheduling_latency'])
                    record['mean_reasoning_time'] = safe_mean(df_latency['reasoning_time'])
                    ...
                    if topology == 'hierarchical' and 'leader_id' in df_latency.columns:
                        ...
                        for level in sorted(df_latency['hierarchy_level'].unique()):
                            ...
                            record[f'level{level}_mean_scheduling_latency'] = safe_mean(df_level_lat['scheduling_latency'])
                            record[f'level{level}_median_scheduling_latency'] = safe_median(df_level_lat['scheduling_latency'])
