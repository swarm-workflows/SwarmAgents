...
class Agent(Observer):
    ...
    def is_job_feasible(self, job: Job, total: Capacities, projected_load: float,
        ...
        self.logger.info(f"KOMAL -is_job_feasible: projected_load-- {projected_load}")
    ...
    def compute_overall_load(self, proposed_jobs: List[str] = None):
        ...
        self.logger.info(f"KOMAL -Ready Queue-- {self.ready_queue.jobs.keys()}")
        self.logger.info(f"KOMAL -Select Queue-- {self.selected_queue.jobs.keys()}")
